...
@Component({
  selector: 'soupes',
  templateUrl: './soupes.component.html',
  styleUrls: ['./soupes.component.css']
})
export class SoupesComponent implements OnInit {
  ...
  constructor(private http: HttpClient,private token: TokenStorageService, private demande: DemandesService,private plat: PlatsService,private favoris: FavorisService) {
   ...
   this.plat.getplat(8,this.username)
   .subscribe(response =>{      
  ...
  onClickdelete(post){
   ...
   .subscribe(ra =>{
    this.plat.getplat(8,this.username).subscribe(response =>{      
      this.posts = response as any ;
    })
  });}
