...
@Component({
  selector: 'woks',
  templateUrl: './woks.component.html',
  styleUrls: ['./woks.component.css']
})
export class WoksComponent implements OnInit {
  ...
  username ;
    ...
    constructor(private http: HttpClient,private token: TokenStorageService, private demande: DemandesService,private plat: PlatsService,private favoris: FavorisService) {
     ...
     this.plat.getplat(9,this.username)
     .subscribe(response =>{      
    ...
    onClickdelete(post){
     ...
     .subscribe(ra =>{
      this.plat.getplat(9,this.username).subscribe(response =>{      
        this.posts = response as any ;
      })
    });}
