...
@Component({
  selector: 'poulet',
  templateUrl: './poulet.component.html',
  styleUrls: ['./poulet.component.css',]
})
export class PouletComponent implements OnInit {
  ...
  constructor(private http: HttpClient,private token: TokenStorageService, private demande: DemandesService,private plat: PlatsService,private favoris: FavorisService) {
   ...
   this.plat.getplat(2,this.username)
   .subscribe(response =>{      
  ...
  onClickdelete(post){
   ...
   .subscribe(ra =>{
    this.plat.getplat(2,this.username).subscribe(response =>{      
      this.posts = response as any ;
    })
  });}
