...
@Component({
  selector: 'jusfrais',
  templateUrl: './jusfrais.component.html',
  styleUrls: ['./jusfrais.component.css']
})
export class JusfraisComponent implements OnInit {
  ...
  constructor(private http: HttpClient,private token: TokenStorageService, private demande: DemandesService,private plat: PlatsService,private favoris: FavorisService) {
   ...
   this.plat.getplat(14,this.username)
   .subscribe(response =>{      
  ...
  onClickdelete(post){
   ...
   .subscribe(ra =>{
    this.plat.getplat(14,this.username).subscribe(response =>{      
      this.posts = response as any ;
    })
  });}
